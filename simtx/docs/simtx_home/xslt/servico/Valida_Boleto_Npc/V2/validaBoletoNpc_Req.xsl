		...
		<multicanal:SERVICO_ENTRADA 
			...
			<xsl:if test="$agendamento = 'sim'">
				<calculaboleto:ROTINAS_USO_COMUM_ENTRADA>
					...
					<xsl:apply-templates mode="copy" select="$saidaBoleto/TITULO/JUROS" disable-output-escaping="yes"/>
					<xsl:apply-templates mode="copy" select="$saidaBoleto/TITULO/MULTA" disable-output-escaping="yes"/>
					<xsl:apply-templates mode="copy" select="$saidaBoleto/TITULO/DESCONTOS" disable-output-escaping="yes"/>
			...
			<valida:VALIDA_REGRAS_BOLETO_ENTRADA>
				...
				<xsl:choose>
					<xsl:when test="$transacao_origem/*[local-name()='CONSULTA_BOLETO_SAIDA']/FLAG_NOVA_COBRANCA">
						<NOVA_PLATAFORMA_DE_COBRANCA><xsl:value-of select="$transacao_origem/*[local-name()='CONSULTA_BOLETO_SAIDA']/FLAG_NOVA_COBRANCA" /></NOVA_PLATAFORMA_DE_COBRANCA>
					</xsl:when>
					<xsl:otherwise>
