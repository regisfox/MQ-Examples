...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:param name="pDataInicioDefault"/>
	<xsl:param name="pDataTerminDefault"/>
	...
	<xsl:template match="/BUSDATA">
		<multicanal:SERVICO_ENTRADA 
			...
			<resumo:CONSULTA_TRANSACOES_CONTA_ENTRADA>
				...
				<PERIODO>
					<INICIO>
						<xsl:if test="$pDataInicioDefault != ''"><xsl:value-of select="$pDataInicioDefault" /></xsl:if>
						<xsl:if test="$pDataInicioDefault = ''"><xsl:value-of select="*[2]/LISTA_AGENDAMENTOS_TRANSACOES/PERIODO/DATA_INICIO" /></xsl:if>					
					</INICIO>
					<FIM>
						<xsl:if test="$pDataTerminDefault != ''"><xsl:value-of select="$pDataTerminDefault" /></xsl:if>
						<xsl:if test="$pDataTerminDefault = ''"><xsl:value-of select="*[2]/LISTA_AGENDAMENTOS_TRANSACOES/PERIODO/DATA_FIM" /></xsl:if>					
					</FIM>
				</PERIODO>
				<SOLICITACOES>
					<SOLICITACAO><xsl:value-of select="*[2]/LISTA_AGENDAMENTOS_TRANSACOES/IDENTIFICADOR_SERVICO" /></SOLICITACAO>
				</SOLICITACOES>
			</resumo:CONSULTA_TRANSACOES_CONTA_ENTRADA>
