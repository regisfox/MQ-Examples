...
<xsl:stylesheet version="2.0"
	...
	<xsl:template match="/BUSDATA">
		<ns:LISTA_COMPROVANTES_TRANSACOES_SAIDA
			xmlns:ns="http://caixa.gov.br/simtx/lista_comprovantes_transacoes/v1/ns"
			xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" 
			xsi:schemaLocation="http://caixa.gov.br/simtx/lista_comprovantes_transacoes/v1/ns Lista_Comprovantes_Transacoes.xsd ">
			<HEADER>
				<SERVICO>
			...
			<xsl:if test="$erro = ''">
				...
				<xsl:if test="$acaoRetorno != 'IMPEDITIVA'">
					<LISTA_COMPROVANTES_TRANSACOES>
						<NOME_SERVICO_SOLICITADO>BOLETO</NOME_SERVICO_SOLICITADO>
						<TRANSACOES>
							<xsl:for-each select="*[local-name()='SERVICO_SAIDA']/*[local-name()='CONSULTA_TRANSACOES_CONTA_SAIDA']/BOLETO/TRANSACOES/*">
								<TRANSACAO>
									<SISTEMA_ORIGEM>SIAUT</SISTEMA_ORIGEM>
							        <NSU><xsl:value-of select="./NSU"/></NSU>
							        <VALOR><xsl:value-of select="./VALOR"/></VALOR>
							        <DATA_MOVIMENTO><xsl:value-of select="./DATA_PAGAMENTO"/></DATA_MOVIMENTO>
							        <DATA_EFETIVACAO><xsl:value-of select="./DATA_EFETIVACAO"/></DATA_EFETIVACAO>
							        <IDENTIFICACAO><xsl:value-of select="./IDENTIFICADOR"/></IDENTIFICACAO>
							        <xsl:choose>
							        	<xsl:when test="./SITUACAO = 'Estornado'">
							        		<IDENTIFICADOR_CANCELAMENTO>SIM</IDENTIFICADOR_CANCELAMENTO>
							        	</xsl:when>
							        	<xsl:otherwise>
							        		<IDENTIFICADOR_CANCELAMENTO>NAO</IDENTIFICADOR_CANCELAMENTO>
							        	</xsl:otherwise>
							        </xsl:choose>
							        <CODIGO_SERVICO_DETALHE>110027</CODIGO_SERVICO_DETALHE>
						        </TRANSACAO>
							</xsl:for-each>
							<xsl:for-each select="*[local-name()='LISTA_COMPROVANTES_TRANSACOES_SAIDA']/*[local-name()='LISTA_COMPROVANTES_TRANSACOES']/TRANSACOES/*">
								<TRANSACAO>
									<SISTEMA_ORIGEM>SIMTX</SISTEMA_ORIGEM>
							        <NSU><xsl:value-of select="./NSU_SIMTX"/></NSU>
							        <VALOR><xsl:value-of select="./VALOR"/></VALOR>
							        <DATA_MOVIMENTO><xsl:value-of select="./DATA"/></DATA_MOVIMENTO>
							        <DATA_EFETIVACAO><xsl:value-of select="./DATA"/></DATA_EFETIVACAO>
							        <IDENTIFICACAO><xsl:value-of select="./IDENTIFICACAO"/></IDENTIFICACAO>
							        <xsl:choose>
							        	<xsl:when test="./IDENTIFICADOR_CANCELAMENTO = '1'">
							        		<IDENTIFICADOR_CANCELAMENTO>SIM</IDENTIFICADOR_CANCELAMENTO>
							        	</xsl:when>
							        	<xsl:otherwise>
							        		<IDENTIFICADOR_CANCELAMENTO>NAO</IDENTIFICADOR_CANCELAMENTO>
