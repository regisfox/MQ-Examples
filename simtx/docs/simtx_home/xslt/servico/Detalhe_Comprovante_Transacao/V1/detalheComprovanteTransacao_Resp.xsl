...
<xsl:stylesheet version="2.0"
	...
	<xsl:template match="/BUSDATA">
		<ns:DETALHE_COMPROVANTE_BOLETO_SAIDA
			xmlns:header="http://caixa.gov.br/simtx/comuns/header" 
			xmlns:comuns="http://caixa.gov.br/simtx/comuns/comuns" 
			xmlns:ns="http://caixa.gov.br/simtx/detalhe_comprovante_transacao/boleto/v1/ns"
			xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" 
			xsi:schemaLocation="http://caixa.gov.br/simtx/detalhe_comprovante_transacao/boleto/v1/ns Detalhe_Comprovante_Boleto.xsd ">
			<HEADER>
				<SERVICO>
			...
			<xsl:if test="$erro = ''">
				...
				<xsl:if test="*[local-name()='SERVICO_SAIDA']/*[local-name()='CONSULTA_TRANSACOES_CONTA_SAIDA'] != ''">
				...
				<xsl:if test="*[local-name()='DETALHE_COMPROVANTE_TRANSACAO_SAIDA'] != ''">
					<INFORMACOES_COMPROVANTE>
						<xsl:apply-templates mode="copy" select="*[local-name()='DETALHE_COMPROVANTE_TRANSACAO_SAIDA']/INFORMACOES_COMPROVANTE/*[local-name()='PAGAMENTO_BOLETO_SAIDA']/INFORMACOES_BOLETO/*" disable-output-escaping="yes"/>
					</INFORMACOES_COMPROVANTE>
				</xsl:if>
