...
<xsl:stylesheet version="2.0"
	...
	<xsl:template match="/BUSDATA">
		<multicanal:SERVICO_ENTRADA 
			...
			xmlns:consultaferiado="http://caixa.gov.br/sibar/rotinas_uso_comum/consulta_feriado" 
			xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
			
			<xsl:apply-templates mode="copy" select="*[local-name()='SERVICO_ENTRADA']/*[local-name()='HEADER']" disable-output-escaping="no" />
			<xsl:apply-templates mode="copy" select="*[local-name()='SERVICO_ENTRADA']/*[local-name()='ROTINAS_USO_COMUM_ENTRADA']" disable-output-escaping="no" />
		</multicanal:SERVICO_ENTRADA>
	</xsl:template>
</xsl:stylesheet>